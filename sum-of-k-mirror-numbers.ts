...
// Psuedo Code:
// build a lookup table of k-mirror numbers up to 30 numbers
// iterate through the lookup table and sum the numbers
// return the sum

// k-mirror numbers (first 30 for each base 2-9)
export const mirrorTable: Record<number, number[]> = {
  2: [
...
function kMirror(k: number, n: number): number {
  const kMirrorNumbers = mirrorTable[k];
  return kMirrorNumbers.slice(0, n).reduce((acc, curr) => acc + curr, 0);
};